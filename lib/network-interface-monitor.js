...
/**
 * Utility method to print messages in console with timestamp.
...
const finalizer = new FinalizationRegistry(monitorData => {
    if (monitorData.handle !== null) {
        print(`NetworkInterfaceMonitor on "${monitorData.networkInterface}" for address family ${monitorData.addressFamily} is not properly stopped! You should call NetworkInterfaceMonitor.stop() to stop monitoring a network interface when it is no longer needed!`);
